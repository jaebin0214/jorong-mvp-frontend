...
API 주소가 비어 있는 개발 시연에서는 [admin.html](../admin.html)의 `LIVE` 종목을 [local-admin-market-bridge.js](../scripts/local-admin-market-bridge.js)가 읽어 거래소에 적용합니다. 같은 브라우저·같은 사이트 주소에서 관리자 페이지와 거래소를 열어야 브라우저 저장소가 공유됩니다.

- 초기 관리자 데이터에는 `훈이` LIVE 종목 하나만 있으며, 댓글·사용자·운영 기록·참여 추세는 비어 있습니다.
- 확인할 때는 먼저 `admin.html`을 연 뒤, **동일한 주소**의 `index.html`을 여세요. 예: `http://localhost:5500/admin.html`과 `http://localhost:5500/index.html` 또는 같은 Vercel 배포 주소입니다. `file:///` 주소나 서로 다른 포트는 저장소를 공유하지 않을 수 있습니다.
- 관리자에서 저장·거래 시작 후 거래소 탭은 자동 새로고침되어 최신 LIVE 종목을 표시합니다.
- 반영 범위는 종목명, 이미지, 소개, 기준 가격, 회차 ID, 시작/종료 시각, 다음 예약 시작 시각입니다.
- 기존 LIVE 종목은 일반 편집이 막혀 있으므로, 테스트하려면 기존 종목을 종료한 뒤 편집한 예약 종목을 거래 시작해야 합니다.
- 이 기능은 `window.JORONG_API_BASE_URL`이 설정되면 자동으로 비활성화됩니다. 실서비스에서는 백엔드가 내려주는 현재 시장 데이터가 유일한 기준입니다.
