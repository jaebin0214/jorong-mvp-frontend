...
차트는 투자 기록을 프론트엔드에서 임의로 계산하지 않고, 백엔드가 특정 거래 회차의 저장된 투자 기록을 시간 단위로 집계한 결과를 받습니다.
...
- `open`, `high`, `low`, `close`: 해당 시간 구간의 가격입니다.
- `volume`: 해당 구간에 체결된 총 투자 금액입니다.
- `startedAt`, `endedAt`: ISO 8601 시각 문자열입니다.

## 백엔드 집계 기준

1. `investments` 테이블을 `marketSessionId`, `targetId`, 생성 시각으로 조회합니다.
...
5. 현재 진행 중인 회차에서는 새 투자 처리 후 이 API가 최신 값을 반환하도록 합니다.
