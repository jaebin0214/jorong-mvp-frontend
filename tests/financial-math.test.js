...
test('조롱 3,000 크레딧 / 평균 단가 930 / 현재가 900 예시', () => {
  ...
  assert.equal(math.formatCredits(metrics.unrealizedPnl), '+97 크레딧');
  assert.equal(math.formatRate(metrics.pnlRate), '+3.23%');
  assert.equal(math.formatCreditsUnsigned(metrics.estimatedSettlementAmount), '3,097 크레딧');
});

test('조롱 3,000 크레딧 / 평균 단가 930 / 마감가 860 정산 예시', () => {
  const position = math.calculatePositionAfterOrder({ side: 'MOCK', investmentAmount: 3000, executionPrice: 930 });
  const settlement = math.calculateSettlement({ position, closePrice: 860, balanceBeforeSettlement: 97000 });

  assert.equal(math.formatCredits(settlement.realizedPnl), '+226 크레딧');
  assert.equal(math.formatRate(settlement.pnlRate), '+7.53%');
  assert.equal(math.formatCreditsUnsigned(settlement.settlementAmount), '3,226 크레딧');
...
test('동일 의견 추가 투자 시 가중평균 단가를 사용한다', () => {
  ...
  assert.equal(math.formatPrice(second.averagePrice), '909.09 크레딧');
});
...
test('옹호 포지션은 가격 상승 시 평가수익이 난다', () => {
  ...
  assert.equal(math.formatCredits(metrics.unrealizedPnl), '+600 크레딧');
...
test('최대 손실에서도 정산 지급액은 0 크레딧 아래로 내려가지 않는다', () => {
  const position = math.calculatePositionAfterOrder({ side: 'MOCK', investmentAmount: 3000, executionPrice: 1000 });
  const settlement = math.calculateSettlement({ position, closePrice: 3000, balanceBeforeSettlement: 7000 });

  assert.equal(math.formatCreditsUnsigned(settlement.settlementAmount), '0 크레딧');
